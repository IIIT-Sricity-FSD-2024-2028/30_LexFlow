...
const currentUser = {
  ...
  avatar: (currentUserData.fullName || currentUserData.name || 'L').split(' ').map(w => w[0]).join('').substring(0, 2).toUpperCase(),
...
async function initTasks() {
  try {
    // 1. Fetch Cases (needed for associating tasks with cases)
    allCases = ((casesStorage && (await casesStorage.getCases())) || []);

    // 2. Fetch Tasks from API
    if (tasksAPI) {
        // Lawyers should see tasks assigned to them
        const filters = { assignedUser: currentUser.name };
        if (currentUser.firmId) filters.firmId = currentUser.firmId;
        
        allTasks = await tasksAPI.getAll(filters, currentUser.role);
    } else {
...
function getInitials(t) {
  return t
    ? t.split(" ").map((t) => t[0]).join("").substring(0, 2).toUpperCase()
    : "U";
}
...
function renderTaskRow(t) {
  ...
  return `
    <tr class="page-item">
        ...
        <td>
            <span class="task-date">${fmtTaskDate(t.dueDate)}</span>
        </td>
        <td>
...
(searchInput.addEventListener("input", applyFilters),
  ...
  (window.openViewTaskModal = function (t) {
    ...
    ((document.getElementById("viewTaskId").textContent = e.id),
      ...
      (document.getElementById("viewTaskDate").textContent = fmtTaskDate(e.dueDate)),
  ...
  (window.saveTaskModal = async function () {
    ...
    try {
      if (s) {
        await tasksAPI.update(s, payload, currentUser.role);
      } else {
        await tasksAPI.create(payload, currentUser.role);
      }
  ...
  (window.markTaskAsCompleted = async function (t) {
    ...
    try {
      await tasksAPI.update(e, { status: "Completed" }, currentUser.role);
  ...
  (window.deleteTask = async function (t) {
    if (confirm("Are you sure you want to delete task " + t + "?")) {
      try {
        await tasksAPI.remove(t, currentUser.role);
        await initTasks();
      } catch (err) {
